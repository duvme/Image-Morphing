...
class MorphingTestSuite(unittest.TestCase, ImageAssertion):
    ...
    def test_Triangle(self):
        ...
        with self.subTest(key="Point Content"):             # 11 Points
            ...
            actualValue = sorted([tuple(t) for t in tri.getPoints().tolist()])
            expectedValue = self.support.samplePoints
...
class Support:
    ...
    def __init__(self):
        ...
        with np.load(filePath) as dataFile:
            ...
            self.samplePoints = sorted([tuple(t) for t in dataFile["samplePoints"].tolist()])
